...
public class User_Res extends FragmentActivity {
    ...
    private void bindRecyclerAdapter2(ArrayList<ResDTO> dtos){
        Res_RecyclerViewAdapter adapter2 = new Res_RecyclerViewAdapter(dtos);
    ...
    private void getResTime(){
        ...
        Call<ResResult> call = service.getResTime("U00001",3);
    ...
    private void getResTime2(){
        builder = RetrofitBuilder.getInstance("http://222.100.239.140:11005/");
        service = RetrofitBuilder.getRetrofitService();
        Call<ResResult> call = service.getResTime2("U00001",5);
        call.enqueue(new Callback<ResResult>() {
            @Override
            public void onResponse(Call<ResResult> call2, Response<ResResult> response) {
                if(response.isSuccessful()) {
                    ArrayList<ResDTO> dtos = new ArrayList<>();
                    for(int i=0; i<response.body().getResult().size(); i++) {
                        ResDTO dto = new ResDTO();
